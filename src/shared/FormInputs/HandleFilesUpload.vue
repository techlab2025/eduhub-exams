...
onMounted(() => {
  ...
  const base64List = Array.isArray(props.base64File)
    ? props.base64File
    : props.base64File ? [props.base64File] : [];
...
<template>
  <div class="file-upload-wrapper">
    ...
    <label v-if="!haveContent" :for="inputId" :class="[className ?? 'upload-area', { disabled: isMaxReached }]">
      <span class="upload-icon">↑</span>
      <span class="upload-text">
        {{ isMaxReached ? `Max ${maxFiles} files reached` : 'Click to upload' }}
      </span>
      <input :id="inputId" type="file" :accept="accept" :disabled="isMaxReached" :multiple="multiple" hidden
    ...
    <div v-if="files.length" class="preview-grid">
      <div v-for="file in files" :key="file.id" class="preview-item" title="Click to download"
