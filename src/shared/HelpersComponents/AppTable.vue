        ...
        <tbody>
          ...
          <tr v-if="items.length === 0">
            <td :colspan="headers.length +
              ...
              (selectable ? 1 : 0) +
              ($slots.actions ? 1 : 0)
              " class="empty-row" @dblclick="copyToClipboard(emptyMessage || 'No data available')">
              {{ emptyMessage || 'No data available' }}
            </td>
          </tr>

          <tr v-for="(item, index) in items" :key="getRowKey(item, index)"
            :class="{ selected: selectable && isRowSelected(index) }" @click="emit('row-click', item, index)">
            <!-- Selection checkbox -->
            ...
            <td v-for="header in headers" :key="header.key" :style="{ textAlign: header.align || 'left' }"
              class="td-data" @dblclick.stop="copyToClipboard((item as any)?.[header.key])">
              <!--
                Use named slot `cell-{key}` for custom rendering,
                otherwise fall back to item[key]
              -->
              <slot :name="`cell-${header.key}`" :item="item" :index="index" :value="(item as any)[header.key]">
