...
import EmailRepository from "./country.repository";
...
describe("EmailRepository", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(repository as any, "apiService", "get").mockReturnValue(
      mockApiService,
    );
  });
  ...
  describe("index - fetch list", () => {
    it("should return DataSuccess with parsed email list", async () => {
      ...
      const mockResponse =
        EmailTestFactory.createEmailListApiResponse(mockEmails);
    ...
    it("should return DataFailed on API error", async () => {
      const errorResponse = EmailTestFactory.errorApiResponse(
    ...
    it("should call apiService.index with correct parameters", async () => {
      ...
      expect(mockApiService.index).toHaveBeenCalledWith(
        mockParams,
        mockOptions,
      );
    });
  ...
  describe("update - update existing item", () => {
    ...
    it("should call apiService.update with correct params", async () => {
      ...
      const params = {
        toMap: () => ({ id: 15, email: "update@example.com" }),
        validate: () => ({ isValid: true, errors: [] }),
        validateOrThrow: () => {},
      };
  ...
  describe("delete - delete item", () => {
    ...
    it("should call apiService.delete with correct params", async () => {
      ...
      expect(mockApiService.delete).toHaveBeenCalledWith(params, {
        auth: true,
      });
    });
  });
  ...
  describe("error handling and edge cases", () => {
    describe("malformed data handling", () => {
      ...
      it("should handle parseList with mix of valid and invalid items", () => {
        ...
        expect(
          parsed.every((item: EmailModel) => item instanceof EmailModel),
        ).toBe(true);
      });
    ...
    describe("concurrent operations", () => {
      it("should handle multiple concurrent index calls", async () => {
        const response1 = EmailTestFactory.createEmailListApiResponse([
          EmailTestFactory.createMockEmailJson({ id: 1 }),
        ]);
        const response2 = EmailTestFactory.createEmailListApiResponse([
          EmailTestFactory.createMockEmailJson({ id: 2 }),
        ]);
      ...
      it("should handle concurrent show calls for different IDs", async () => {
        const response1 = EmailTestFactory.createEmailApiResponse(
          EmailTestFactory.createMockEmailJson({ id: 1 }),
        );
        const response2 = EmailTestFactory.createEmailApiResponse(
          EmailTestFactory.createMockEmailJson({ id: 2 }),
        );
