...
<template>
  <DataStatusBuilder :controller="itemState" :on-retry="retryFetchStudent" use-skeleton>
    <template #success>
      <main v-if="student" class="student-details-page">
        ...
        <section class="student-details-content">
          <article class="student-content-card student-account-card">
            ...
            <p class="student-account-message" :class="{ blocked: student.status === StudentStatusEnum.BLOCK }">
          ...
          <article class="student-content-card">
            <h2 class="student-section-title"><IconPerformance />{{ $t('performance_snapshot') }}</h2>
          ...
          <article class="student-content-card student-results-card">
            <header class="student-card-heading">
              ...
              <button type="button" class="student-show-button" @click="showAllPlacementTests = !showAllPlacementTests">
          ...
          <article class="student-content-card student-results-card">
            <header class="student-card-heading">
              ...
              <button type="button" class="student-show-button" @click="showAllPractices = !showAllPractices">
      ...
      <StudentArchiveDialog v-model="archiveDialogVisible" :loading="statusActionLoading" @confirm="confirmArchive" />
      <StudentBlockDialog v-model="blockDialogVisible" :loading="statusActionLoading" @confirm="confirmBlock" />
      <StudentForceLogoutDialog v-model="forceLogoutDialogVisible" :loading="forceLogoutLoading"
        @confirm="confirmForceLogout" />
      <StudentNoteDialog v-model="noteDialogVisible" :student-id="student?.id ?? null" :loading="noteActionLoading"
        @save="addNote" />
    </template>
  </DataStatusBuilder>
</template>
