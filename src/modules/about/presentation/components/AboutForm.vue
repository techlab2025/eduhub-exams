...
const socialMediaList = ref<SocialModel[]>([
  { link: '', icon: '' },
]);
...
watch(
  ...
  (newAbout) => {
    if (newAbout) {
      ...
      if ((newAbout as any).socialMedia?.length) {
        socialMediaList.value = (newAbout as any).socialMedia.map((item: any) => ({
          link: item.link ?? '',
          icon: item.icon ?? '',
        }));
      }
    ...
    <div class="form-fields">
      ...
      <div class="field-group col-span-2">
        <MultiLangInput :field-key="`title`" :label="$t(`title`)" :languages="['en', 'ar']" :model-value="title"
          @update:model-value="title = $event" :type="`title`" />
      </div>
      ...
      <!-- Image Upload -->
      <div class="field-group col-span-2">
        <HandleFilesUpload :label="`upload image`" accept="image/*" :multiple="false" :index="1" :file="image"
      ...
      <div class="field-group col-span-2 social-media-section">
        ...
        <div class="social-media-header">
          ...
          <button type="button" class="reset-btn" @click="resetSocialMedia">
            Reset
          </button>
        </div>
        ...
        <div v-for="(entry, index) in socialMediaList" :key="index" class="social-media-row">
          ...
          <button v-if="index === socialMediaList.length - 1" type="button" class="sm-add-btn"
            @click="addSocialMediaEntry" title="Add another link">
            <span>+</span>
          </button>
          <button v-else type="button" class="sm-remove-btn" @click="removeSocialMediaEntry(index)"
