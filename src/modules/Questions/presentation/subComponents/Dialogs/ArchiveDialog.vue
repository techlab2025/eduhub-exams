...
<template>
  ...
  >
    <div class="dialog-content">
      <img class="dialog-illustration" :src="ArchiveIcon" alt="" />
      <div class="dialog-message">
        <h2>{{ $t('archive_question_dialog.title') }}</h2>
        <p>{{ $t('archive_question_dialog.description') }}</p>
      </div>
      <div class="btns">
        <button type="button" class="btn btn-primary" @click="$emit('archive')">
          {{ $t('archive_question_dialog.confirm') }}
        </button>
        <button type="button" class="btn btn-secondary" @click="visable = false">
          {{ $t('archive_question_dialog.cancel') }}
        </button>
      </div>
    </div>
  </Dialog>
