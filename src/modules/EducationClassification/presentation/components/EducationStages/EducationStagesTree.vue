...
function getBranchName(parentDepth: number): string {
    ...
    return branch.translation.SingularTitle[lang] ?? branch.translation.SingularTitle['en'] ?? `Branch ${parentDepth + 1}`;
}
...
async function fetchRoot() {
    ...
    if (rootNodes.value.length == 0) {
        rootNodes.value = data.map((s) => makeNode(s, 0));
    }
}
...
async function expandNode(node: StageNode) {
    ...
    await controller.fetchList(params);
    const data = controller.listState.value.data ?? [];
    node.children = data.map((s) => makeNode(s, node.depth + 1));
    node.isLoaded = true;
...
async function selectNode(node: StageNode) {
    ...
    const result = await controller.newfetchList(new FetchEducationStageParams({ classification_id: classificationId, parent_id: node.stage.stage_id }));
...
<template>
        ...
        <div class="left-panel">
            <div class="toolbar">
                <div class="search-box">
                    ...
                    <input type="text" placeholder="Search About Education Classification,Branch,Sub..."
                        class="search-input" />
                </div>
                <button class="filter-btn">
                    <svg viewBox="0 0 20 20" fill="none" width="16" height="16">
                        <path d="M3 5h14M6 10h8M9 15h2" stroke="currentColor" stroke-width="1.5"
            ...
            <div v-else class="tree-list">
                <StageTreeNode v-for="node in rootNodes" :key="node.stage.stage_id" :node="node"
                ...
                <div class="right-header">
                    <div class="right-title">
                        <svg viewBox="0 0 20 20" fill="none" width="16" height="16">
                            ...
                            <path d="M7 7h6M7 10h6M7 13h4" stroke="#4caf50" stroke-width="1.2" stroke-linecap="round" />
                        </svg>
                        <span>{{ selectedNode.stage.stage_title }}</span>
                    </div>
                    <button v-if="selectedNode.depth < maxDepth" class="btn-add-branch"
                        @click="openAddChildDialog(selectedNode.stage.stage_id, selectedNode.depth + 1)">
                        ...
                        Add {{ getBranchName(selectedNode.depth) }}
                    </button>
                </div>

                <!-- Children list -->
                <div v-if="selectedNode.children.length > 0" class="right-children">
                    <div v-for="child in selectedNode.children" :key="child.stage.stage_id" class="right-child-row">
                        <svg viewBox="0 0 20 20" fill="none" width="16" height="16" class="child-icon">
                            <rect x="4" y="3" width="12" height="14" rx="2" stroke="#4caf50" stroke-width="1.3"
                                fill="none" />
                        ...
                        <span class="spacer" />
                        <button v-if="child.depth < maxDepth" class="icon-btn"
                            @click="openAddChildDialog(child.stage.stage_id, child.depth + 1)">
                            <svg viewBox="0 0 20 20" fill="none" width="16" height="16">
    ...
    <AddEducationTypeDialog :header="`Add New ${getBranchName(0)}`" v-if="showAddTypeDialog"
        v-model:visible="showAddTypeDialog" @confirm="handleAddType" />

    <AddBranchDialog v-if="showAddBranchDialog" v-model:visible="showAddBranchDialog" :level="branchDialogLevel"
        :branch-id="branchDialogParentId" :branch-name="branchDialogName" @confirm="handleAddBranch" />
</template>
