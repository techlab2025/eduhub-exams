...
const faqs = computed<FaqsModel[]>(() => {
  ...
  if (state instanceof DataSuccess) return (state.data as FaqsModel[]) ?? [];
  return [];
});
          ...
          <div class="faq-row-left">
            <button type="button" class="expand-btn" :class="{ expanded: expandedIndex === idx }">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="12" cy="12" r="10" stroke="#4faf7c" stroke-width="1.5"/>
                <path d="M12 8V16M8 12H16" stroke="#4faf7c" stroke-width="1.5" stroke-linecap="round"/>
              </svg>
            </button>
          ...
          <div class="faq-row-actions" @click.stop>
            ...
            <button type="button" class="action-btn delete-btn" @click="deleteFaq(faq.id!)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M8 6V4C8 3.45 8.45 3 9 3H15C15.55 3 16 3.45 16 4V6M19 6L18.12 19.13C18.05 20.15 17.23 20.94 16.21 20.94H7.79C6.77 20.94 5.95 20.15 5.88 19.13L5 6H19Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </button>
          </div>
        </div>

        <div v-show="expandedIndex === idx" class="faq-answer">
          <p>{{ getAnswer(faq) }}</p>
        </div>
