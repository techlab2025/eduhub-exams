<script setup lang="ts">
  ...
  import ActiveAndDeactiveNotificationPlanDialog from '../subCopmnent/ActiveAndDeactiveNotificationPlanDialog.vue';
  ...
  // const isPlanActivete = ref<boolean>(false);

  // const OpenActiveateDialog = (item: NotificationPlanModel) => {
  //   DialogVisiable.value = true;
  //   isPlanActivete.value = isPlanActive(item);
  //   // changeStatus(item);
  // };
  ...
  // هنا القيمة معناها:
  // true  => هنفعل الـ plan
  // false => هنعطل الـ plan
  const isPlanActivete = ref<boolean>(false);

  const OpenActiveateDialog = (item: NotificationPlanModel) => {
    selectedPlan.value = item;

    // target status وليس current status
    isPlanActivete.value = !isPlanActive(item);
  ...
  const confirmChangeStatus = async () => {
    ...
    await changeStatus(selectedPlan.value);
  ...
  const actionList = (item: NotificationPlanModel) => [
    ...
    },
    {
      ...
      action: () => OpenActiveateDialog(item),
...
<template>
          ...
          >
            ...
            <template #actions="{ item }">
              <DropList
                ...
                :delete-dialog-title="$t('Are you sure you want to delete this notification plan?')"
                :delete-dialog-message="
                  $t(
                    'this action cannot be undone. The plan will be permanently deleted, and its configured triggers will become available for other notification plans.',
                  )
                "
                :deleteDialogClass="`notification-plan-delete-dialog`"
              >
                <template #icon>
                  <svg
                    width="4"
  ...
  <ActiveAndDeactiveNotificationPlanDialog
    :visable="DialogVisiable"
    :is-activate="isPlanActivete"
    @confirm-data="confirmChangeStatus"
